...
const productSchema = z.object({
  ...
  variety: z.string().min(1, 'Varietas bahan baku wajib dipilih'),
...
export function ProductForm({ initialData, isEdit = false }: ProductFormProps) {
  ...
  const form = useForm<ProductFormData>({
    ...
    defaultValues: {
      ...
      category: initialData?.category ?? 'olahan',
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    try {
      ...
      window.location.href = '/umkm/produk'
    } catch {
  ...
  return (
    ...
    >
      ...
      {/* Nama Produk */}
      <div className="space-y-2">
        <Label htmlFor="name">Nama Produk Olahan</Label>
        <Input id="name" placeholder="Contoh: Keripik Mangga Original" {...form.register('name')} />
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Select value={form.watch('variety')} onValueChange={(v) => form.setValue('variety', v)}>
            <SelectTrigger>
              <SelectValue placeholder="Pilih varietas mangga" />
            </SelectTrigger>
            <SelectContent>
      ...
      <div className="space-y-2">
        ...
        <Textarea id="description" rows={4} placeholder="Deskripsikan produk olahanmu..." {...form.register('description')} />
